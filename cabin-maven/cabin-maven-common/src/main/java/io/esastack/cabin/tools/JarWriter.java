...
public class JarWriter implements LoaderClassesWriter, Closeable{
     ...
     */
    private static final String COMMON_ARTIFACT_ID = "cabin-common";

    private static final String ARCHIVE_ARTIFACT_ID = "cabin-archive";

    private static final String BOOT_ARTIFACT_ID = "cabin-boot";
    ...
    public void writeBootstrapEntry(JarFile containerJar) throws IOException {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (entry.getName().contains(ARCHIVE_ARTIFACT_ID)
                    || entry.getName().contains(BOOT_ARTIFACT_ID)
                    || entry.getName().contains(COMMON_ARTIFACT_ID)) {
                JarInputStream inputStream = new JarInputStream(new BufferedInputStream(
                        containerJar.getInputStream(entry)));
    ...
    public void writeNestedLibrary(String destination, Library library) throws IOException {
        ...
        if (library.isUnpackRequired()) {
            entry.setComment("UNPACK:" + FileUtils.digest(new FileInputStream(file), "SHA-1"));
        }
