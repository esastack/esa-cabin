...
final class CentralDirectoryFileHeader implements FileHeader {
    ...
    public static CentralDirectoryFileHeader fromRandomAccessData(
            RandomAccessData data, int offset, JarEntryFilter filter) throws IOException {
    ...
    public CentralDirectoryFileHeader clone() {
        byte[] headerBytes = new byte[46];
        System.arraycopy(this.header, this.headerOffset, headerBytes, 0, headerBytes.length);
        return new CentralDirectoryFileHeader(headerBytes, 0, this.name, headerBytes, this.comment,
